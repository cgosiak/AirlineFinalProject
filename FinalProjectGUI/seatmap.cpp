...
namespace FinalProject {
    ...
    void SeatMap::Set_Seat_Ints(int rows, int seats_per_row) {
        plane_rows = rows;
        plane_seats_per_row = seats_per_row;
    }
    ...
    std::string SeatMap::Get_String_mapping(int use_this_index) {
        std::string return_data;
        return_data = "";
        UsefulFunctions useThis;
        std::string rowToPrint = useThis.getStringfromInt(use_this_index+1) + ":";
        for (int seatNum = 0; seatNum <= plane_seats_per_row; seatNum++) {
            rowToPrint += " " + Seats[use_this_index][seatNum]->Get_seat_letter();
        }
        cout << rowToPrint << std::endl;
        return_data = return_data + rowToPrint + "\n";
        return return_data;
    }

    std::string SeatMap::Return_SeatMapping_String2() {
        return printable_string_map;
    }
    ...
    void SeatMap::Generate_seats() {
        ...
        cout << "MEDJABK: " << plane_rows << std::endl;
        cout << "MEDJABK: " << plane_seats_per_row << std::endl;
    }
    ...
    void SeatMap::Update_String_map() {
        printable_string_map = "1: A B D F";
        cout << printable_string_map << std::endl;
    }
