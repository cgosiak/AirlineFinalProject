...
namespace FinalProject {
    ...
    std::string Airport::Get_Printable_Flight_Row(int cur_index) {
        std::string print_row;
        print_row = "----------------------------------------------------------------------------------------------------------------------------------------------\nFLIGHT NUMBER: " + std::to_string(current_flights[cur_index]->Get_Flight_Num()) + " DESTINATION: " + current_flights[cur_index]->Get_Destination() + "\nDEPARTURE DATE: " + current_flights[cur_index]->Get_Time_Departing() + " ARRIVAL DATE: " + current_flights[cur_index]->Get_Time_Arriving() + "\nSEATS AVAILABLE: "  + std::to_string(current_flights[cur_index]->Get_Seats_Available()) + " ASSIGNED PLANE: "  + std::to_string(current_flights[cur_index]->Get_Plane_Num()) + " DAYS TO FLIGHT: " + std::to_string(current_flights[cur_index]->Get_Days_To_Flight()) + "\n----------------------------------------------------------------------------------------------------------------------------------------------";
        return print_row;
    }
    ...
    std::string Airport::Get_Printable_Passenger_Row(int cur_index) {
        std::string output_row;
        output_row = "----------------------------------------------------------------------------------------------------------------------------------------------\nNAME: " + all_passengers[cur_index]->Get_Last() + ", " + all_passengers[cur_index]->Get_First() + "\nAGE: " + std::to_string(all_passengers[cur_index]->Get_Age()) + "\nFLIGHTS: " + all_passengers[cur_index]->Get_Flight_List() + "\n----------------------------------------------------------------------------------------------------------------------------------------------";
        return output_row;
    }
    ...
    int Airport::Get_Number_of_Passengers() {
        return current_passenger_amount;
    }
    ...
    std::string Airport::Get_Printable_Plane_Row(int cur_index) {
        std::string print_row;
        print_row = "----------------------------------------------------------------------------------------------------------------------------------------------\nPLANE NUMBER: " + std::to_string(planes_at_airport[cur_index]->Get_Flight_Num()) + "\nROWS: " + std::to_string(planes_at_airport[cur_index]->Get_Rows()) + "    SEATS PER ROW: " + std::to_string(planes_at_airport[cur_index]->Get_Seats_Per_Row()) + "\nPLANE ASSIGNED TO FLIGHT: " + std::to_string(planes_at_airport[cur_index]->Is_Plane_Assigned_To_Flight()) + "\n----------------------------------------------------------------------------------------------------------------------------------------------";
        return print_row;
    }
    ...
    bool Airport::Select_Plane(int index_of_plane) {
        ...
        if ((index_of_plane < current_plane_amount) && (index_of_plane >= 0)) {
            ...
            cout << "SUCCESS: PLANE " << selected_plane->Get_Flight_Num() << " SELECTED." << std::endl;
    ...
    bool Airport::Read_External_Passengers() {
        // input file
        std::ifstream myFile ("/home/caleb/Desktop/FinalCS3528/SavedData/passengers.txt");
    ...
    bool Airport::GUI_Add_Passenger(std::string fName, std::string lName, int age, int row, int seat) {
        if (seat > 0 && row > 0) {
            if (Check_For_Duplicate_Passenger(fName,lName,age)) {
                selected_passenger = Get_Duplicate_Passenger(fName,lName,age);

                selected_flight->Add_Passenger_To_Flight(selected_passenger,row,seat);

                selected_flight->assigned_plane->Reserve_From_External_File(row-1, seat-1,
                                                                            selected_flight->most_recently_added);
                return true;
            }
            else {
                selected_flight->Add_Passenger_To_Flight(fName, lName, age, row,
                                                            seat);
                selected_flight->assigned_plane->Reserve_From_External_File((row - 1),
                                                                               (seat - 1),
                                                                               selected_flight->most_recently_added);
                all_passengers[current_passenger_amount] = selected_flight->most_recently_added; // add the most recently added passenger to the list of all passengers
                current_passenger_amount++;
    ...
    bool Airport::Read_External_Amounts() {
        // input file
        std::ifstream myFile ("/home/caleb/Desktop/FinalCS3528/SavedData/planes_and_flights.txt");
        int num_planes = 0;
        int num_flights = 0;
    ...
    bool Airport::Read_External_Planes() {
        ...
        if (current_plane_amount > 0) {
            ...
            for (int i = 0; i < current_plane_amount; ++i) {
                ...
                file_name = "/home/caleb/Desktop/FinalCS3528/SavedData/saved_plane_" + std::to_string(i+1) + ".txt";
    ...
    bool Airport::GUI_Add_Plane(int plane_num, int rows, int seats_per_row) {
        if (Check_For_Duplicate_Plane(plane_num)) {
            return false;
        }
        else {
            planes_at_airport[current_plane_amount] = new Plane(plane_num,rows,seats_per_row);
            current_plane_amount++;
            return true;
        }
    ...
    bool Airport::Read_External_Flights() {
        ...
        if (current_flight_amount > 0) {
            ...
            for (int i = 0; i < current_flight_amount; ++i) {
                std::string file_name;
                file_name = "/home/caleb/Desktop/FinalCS3528/SavedData/saved_flight_" + std::to_string(i+1) + ".txt";
    ...
    bool Airport::GUI_Add_Flight(int flight_num,std::string destination,int depart_year,int depart_month,int depart_day,int depart_hr,int depart_min,int a_yr,int a_mt,int a_dy,int a_hr,int a_min) {
        // check to be sure there is not the same flight num that exists
        if (Check_For_Duplicate_Flight(flight_num)) {
            return false;
        }
        else {
            current_flights[current_flight_amount] = new Flight(flight_num,destination,depart_year,depart_month,depart_day,depart_hr,depart_min,a_yr,a_mt,a_dy,a_hr,a_min,0);
            current_flight_amount++;
        }
        return true;
    }
    ...
    void Airport::Save_States() {
        ...
        std::string path = "/home/caleb/Desktop/FinalCS3528/SavedData/*.txt";
    ...
    void Airport::Save_State_Planes_Flights() {
        std::string file_name;
        file_name = "/home/caleb/Desktop/FinalCS3528/SavedData/planes_and_flights.txt";
    ...
    void Airport::Save_State_Passengers() {
        std::string file_name;
        file_name = "/home/caleb/Desktop/FinalCS3528/SavedData/passengers.txt";
        std::ofstream myFile (file_name.c_str());
